...
public class NyaRemoteProcess extends Process implements Parcelable {

    private static final Set<NyaRemoteProcess> CACHE = Collections.synchronizedSet(new ArraySet<>());

    private static final String TAG = "NyaRemoteProcess";

    private IRemoteProcess remote;
    private OutputStream os;
    private InputStream is;

    public NyaRemoteProcess(IRemoteProcess remote) {
        this.remote = remote;
        try {
    ...
    public NyaRemoteProcess(Parcel in) {
        remote = IRemoteProcess.Stub.asInterface(in.readStrongBinder());
    }

    public static final Creator<NyaRemoteProcess> CREATOR = new Creator<NyaRemoteProcess>() {
        @Override
        public NyaRemoteProcess createFromParcel(Parcel in) {
            return new NyaRemoteProcess(in);
        }

        @Override
        public NyaRemoteProcess[] newArray(int size) {
            return new NyaRemoteProcess[size];
        }
    };
